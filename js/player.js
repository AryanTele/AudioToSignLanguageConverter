...
function isSentenceEndHindi(letter) {
  ...
  if (letter.charCodeAt(0).toString(16) == "964") return true;
  return false;
}
...
function isSentenceEndEnglish(letter) {
  ...
  for (x = 0; x < englishpMarks.length; x++) {
    if (letter == englishpMarks[x]) return true;
  }
  return false;
}
...
function isSpace(letter) {
  ...
  if (letter.charCodeAt(0).toString(16) == "20") return true;
  return false;
}
...
function tokenizeEnglish(inText) {
  ...
  if (flag == false) inputText = inText + ".";
  else inputText = inText;
  ...
  for (y = 0; y < result.length; y++) {
    ...
    for (x = 0; x < line.length; x++) {
      if (isSpace(line[x])) newString = newString + ",";
      else newString = newString + line[x];
    }
...
function searchForSigml(key) {
  $.ajax({
    url: "API/sigmlAPI.php?action=search",
    type: "POST",
    data: { q: key },
    async: false,
